 ...
 *
 * Configure runtime values with environment variables in Cloudflare:
 * - UUID
 * - PROXYIP
 * - SCAMALYTICS_USERNAME / SCAMALYTICS_API_KEY / SCAMALYTICS_BASEURL
 * - SOCKS5 / SOCKS5_RELAY
 */

const DEFAULT_USER_ID = "be0ff9df-1468-41a0-8865-796d1c6800db";
const WORKER_VERSION = "2026-02-20-uuid-hotfix-3";
...
const Config = {
  ...
  fromEnv(env) {
    ...
    const socksEnabled = parseBooleanEnv(env.SOCKS5_ENABLED, undefined);
    const socksRaw = typeof env.SOCKS5 === "string" ? env.SOCKS5.trim() : "";
    const socksFromLegacyFlag = parseBooleanEnv(env.SOCKS5, undefined);
    const socksAddress =
      env.SOCKS5_ADDRESS || (socksRaw.includes(":") || socksRaw.includes("@") ? socksRaw : "");

    // Keep both names intentionally to avoid regressions from stale references.
    const parsedUUIDs = parseUUIDList(env.UUID, this.userID);
    const uuidList = parsedUUIDs;

    return {
      userID: uuidList.join(","),
      primaryUserID: uuidList[0],
      proxyIP: proxyHost,
      proxyPort: proxyPort,
      ...
      socks5: {
        enabled:
          typeof socksEnabled === "boolean"
            ? socksEnabled
            : typeof socksFromLegacyFlag === "boolean"
              ? socksFromLegacyFlag
              : !!socksAddress,
        relayMode: parseBooleanEnv(env.SOCKS5_RELAY, this.socks5.relayMode),
        address: socksAddress || this.socks5.address,
      },
    };
  },
};
...
async function handleIpSubscription(request, core, userID, hostName) {
  ...
  try {
    ...
    if (ipv4Res.ok || ipv6Res.ok) {
      const ipv4Text = ipv4Res.ok ? await ipv4Res.text() : "";
      const ipv6Text = ipv6Res.ok ? await ipv6Res.text() : "";
      const toSamples = (text) =>
        text
          .split("\n")
          .map((line) => line.trim())
          .filter(Boolean)
      ...
      const ips = [...toSamples(ipv4Text), ...toSamples(ipv6Text)].slice(0, 20);
      ips.forEach((ip, i) => {
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (upgradeHeader && upgradeHeader.toLowerCase() === "websocket") {
        const parsedSocks5Address = cfg.socks5.enabled
      ...
      if (url.pathname === "/__version") {
        return Response.json({
          ...
          primaryUserID: cfg.primaryUserID,
          uuidCount: cfg.userID ? String(cfg.userID).split(",").length : 0,
        });
      }

      if (url.pathname.startsWith(`/xray/${cfg.primaryUserID}`))
        return handleIpSubscription(request, "xray", cfg.primaryUserID, url.hostname);

      if (url.pathname.startsWith(`/sb/${cfg.primaryUserID}`))
        return handleIpSubscription(request, "sb", cfg.primaryUserID, url.hostname);

      if (url.pathname === "/" || url.pathname.startsWith(`/${cfg.primaryUserID}`))
        return handleConfigPage(cfg.primaryUserID, url.hostname, cfg.proxyAddress);
...
function generateBeautifulConfigPage(userID, hostName, proxyAddress) {
  ...
  const clientUrls = {
    clashMeta: `clash://install-config?url=${encodeURIComponent(subSbUrl)}&name=${encodedSubName}`,
  ...
  <head>
    ...
    <title>Spring Edge • VLESS Console</title>
    <link rel="icon" href="data:image/svg+xml,%3Csvg xmlns=%27http://www.w3.org/2000/svg%27 viewBox=%270 0 100 100%27%3E%3Ctext y=%27.9em%27 font-size=%2790%27%3E%E2%9A%A1%3C/text%3E%3C/svg%3E" type="image/svg+xml">
...
/**
 * Parse UUID env var and always return at least one valid UUID.
 * Accepts comma/newline/space separated UUID values.
 * @param {unknown} rawValue
 * @param {string} fallbackUserID
 * @returns {string[]}
 */
function parseUUIDList(rawValue, fallbackUserID) {
  ...
  const fallbackList = String(fallbackUserID || "")
    .split(",")
    .map((value) => value.trim())
    .filter((value) => isValidUUID(value));
  return fallbackList.length ? fallbackList : [DEFAULT_USER_ID];
}
...
async function HandleTCPOutBound(
  ...
  async function retry() {
    ...
    RemoteSocketToWS(tcpSocket, webSocket, protocolResponseHeader, null, log).catch((err) => {
      log(`RemoteSocket error: ${err?.message || err}`);
    });
  }

  const tcpSocket = await connectAndWrite(addressRemote, portRemote);
  RemoteSocketToWS(tcpSocket, webSocket, protocolResponseHeader, retry, log).catch((err) => {
    log(`RemoteSocket error: ${err?.message || err}`);
  });
}
...
function ProcessProtocolHeader(protocolBuffer, userID) {
  ...
  const uuids = String(userID || "")
    .split(",")
    .map((/** @type {string} */ id) => id.trim())
    .filter(Boolean);
  if (!uuids.length) return { hasError: true, message: "missing UUID configuration" };
...
async function RemoteSocketToWS(remoteSocket, webSocket, protocolResponseHeader, retry, log) {
  ...
  if (!hasIncomingData && retry) {
    ...
    try {
      ...
      log(`Retry failed: ${retryError?.message || retryError}`);
    }
  }
}
...
function getPageCSS() {
  ...
  * {
  ...
  :root {
    --background-primary: #0b1220; --background-secondary: #121b2b; --background-tertiary: #1a2740;
    --border-color: #2e3d5e; --border-color-hover: #4a6294; --text-primary: #e6efff; --text-secondary: #a8bbdf;
    --text-accent: #ffffff; --accent-primary: #6ba7ff; --accent-secondary: #8bb8ff; --accent-tertiary: #3e78d9;
    --accent-primary-darker: #2f5fae; --button-text-primary: #081225; --button-text-secondary: var(--text-primary);
    --shadow-color: rgba(0, 0, 0, 0.45); --shadow-color-accent: rgba(80, 135, 230, 0.28);
  ...
  body {
    ...
    background: radial-gradient(circle at 15% 15%, #1d2f4f 0%, #0b1220 45%, #070c16 100%); color: var(--text-primary);
    padding: 3rem; line-height: 1.5; -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale;
  }

  .container {
    max-width: 880px; margin: 20px auto; padding: 0 14px; border-radius: 14px;
    border: 1px solid var(--border-color); backdrop-filter: blur(6px);
    background: linear-gradient(145deg, rgba(21,34,57,0.86), rgba(11,18,32,0.92));
    box-shadow: 0 8px 22px rgba(0, 0, 0, 0.32), 0 0 28px 6px var(--shadow-color-accent);
    transition: box-shadow var(--transition-speed-medium) ease, transform var(--transition-speed-medium) ease;
  }

  .container:hover { box-shadow: 0 12px 28px rgba(0, 0, 0, 0.36), 0 0 40px 12px var(--shadow-color-accent); transform: translateY(-2px); }
...
function getPageHTML(configs, clientUrls) {
  return `
    <div class="container">
    <div class="header">
      <h1>Spring Edge Console</h1>
      <p>Manage your VLESS links, network status, and one-click client imports.</p>
    </div>
